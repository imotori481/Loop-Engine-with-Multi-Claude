...
| 未着手 | ソルバーの時間切れを試行の失敗として数える | run 8 の S10 で、2回目の実装のソルバーが制限時間内に終わらず、残りの試行を使わずにエスカレーションした。エスカレーションの上限を使い切り、人間で止まった | Python。IMPL の時間切れだけを、Halt ではなく試行1回分の失敗として扱う。台帳に `SOLVER_TIMEOUT` を残し、書きかけの作業ツリーは捨て、次の試行のブリーフに時間切れだったことを伝える。TEST_WRITE と STUB の時間切れは今までどおり止める。unittest で確かめる | `runner/loop.py`（`call_solver` `run_step`）、`runner/tests/` |
| 未着手 | 開発サーバの振る舞いを確かめる条件を書かせない | run 8 の S10 で、Vite が `index.html` を変換した結果を確かめる条件が書かれた。変換結果は Vite が決め、どの実装でも変えられない。ソルバーは直せない1件のために時間切れまで考えた | Python。TypeScript のとき、環境の事実に「開発サーバで開けることは環境が保証する。要件にあっても、`start` を export するステップで満たされる。Vite の出力を確かめる条件は書かない」と書き足す。unittest でブリーフの文を確かめる | `runner/loop.py`（`environment_facts`）、`runner/tests/test_brief.py` |
| 未着手 | 開発サーバで開けることを箱が確かめる | 上の行で計画から外す要件を、代わりに誰かが確かめる | bash。Vite で `index.html` を変換し、`/src/main.ts` を読み込む経路ができていることを確かめる検査を足す。Vite はインラインのモジュールスクリプトを `html-proxy` に切り出すので、確かめるのは変換後の HTML ではなく、切り出されたモジュールの中身 | `provision/bin/smoke-dom`、`provision/35-node.sh` |
| 未着手 | R4 で空と分かった条件をプランナーが外せるようにする | 適用済みの条件は P2 と P3 でプランナーに変えられず、スタブに対して通ってしまう条件（R4）も必ず人間で止まる。run 8 の S10 では、環境のファイルを確かめる条件のために人間が計画を手で直した | Python。R4 で止まったステップに限り、`check_proposal` の P2 と P3 に例外を設ける。許すのは条件を消すことだけで、残る条件は一字も変えない。消せる数は R4 で通ったテストの数まで。`expected_tests` は消した数と同じだけ下げる。R4 の通ったテストの数は、エスカレーションのときに台帳へ残して、そこから読む。改訂のブリーフにこの手を伝える。unittest で、許す場合と拒む場合（書き換え、数の超過、別のステップ、R4 以外の停止）を確かめる | `runner/loop.py`（`escalate` `check_proposal` `brief_plan_revise`）、`runner/tests/` |
