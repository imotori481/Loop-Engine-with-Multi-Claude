...
| 未着手 | 批評が走らなかったときに clean と表示しない | 要件が `/srv/loop/human/in/REQUIREMENTS.md` に無いと、coverage が黙って飛ばされ、指摘ゼロとして扱われる | `run_critique` の結果が空なら、clean にせずエラーで止める | `runner/loop.py`（`cmd_plan_refine` `cmd_critique`） |
