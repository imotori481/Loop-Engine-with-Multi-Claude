...
| 一部完了 | run 8 の計画を Claude ソルバーで回す | run 5（Codex）、run 6（ローカル 9B）と同じ物差しで比較する | `requirements/IDLE_GAME.md` を TypeScript で `plan bootstrap` → `plan refine` → `plan apply` → `run --all`。計画づくりは完了（bootstrap は1回目でリンタ通過、改訂2回、プランナー6回・クリティック6回）。ステップごとの試行回数と所要時間を記録する | `docs/HANDOFF.md` |
...
| 未着手 | 改訂で CONTEXT.md と SYSTEM_SPEC.md を失わない | `plan refine` の改訂で、プランナーは変える `tasks.json` だけを書き、B1（最初の計画には3ファイルが要る）に落ちる。run 8 で2回起き、プランナーの呼び出しを2回無駄にした | 改訂のあいだは、プランナーが書かなかったファイルを改訂前の控えから補う。または改訂のブリーフで、3ファイルとも書き直すよう伝える | `runner/loop.py`（`cmd_plan_refine` `brief_plan_refine`） |
| 未着手 | `USAGE` の画面表示を短くする | 台帳の1行がそのまま画面に出て、1回の呼び出しで数行を占める | 台帳には全部残し、画面にはモデル・入出力トークン・時間・推定費用だけを出す | `runner/loop.py`（`record_usage` `ledger`） |
| 未着手 | trace に守りのテストを指摘させない | 壊れたセーブや知らない ID への耐性を試すテストを、毎回「遊んでいても届かない状態」と指摘する。run 8 では3回の批評すべてに残った | trace のブリーフで、外から壊された入力への守りは指摘の対象外だと伝える | `runner/loop.py`（`brief_critique_trace`） |
